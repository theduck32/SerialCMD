...
std::string GetOSVersion() {
	HKEY hKey;
	LONG result = RegOpenKeyEx(HKEY_LOCAL_MACHINE, "SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion", 0, KEY_QUERY_VALUE, &hKey);
	if (result != ERROR_SUCCESS) {
		return "Error opening registry key.";
	}
	char value[256];
	DWORD dataSize = sizeof(value);
	result = RegGetValue(hKey, NULL, "BuildLabEx", RRF_RT_REG_SZ, NULL, value, &dataSize);
	RegCloseKey(hKey);
	if (result != ERROR_SUCCESS) {
        	return "Error reading registry value.";
	}
	return value;
}
...
int main() {
	...
	SerialWrite("OS Version: " + osVersion + "\r\n");
